...
def resolve_command(settings) -> tuple[list[str] | None, str]:
    """Resolve the streamer launch command. Returns ``(argv_prefix, detail)``;
    ``argv_prefix`` is None when no runner is available (detail says why).
    ...
    if getattr(settings, "teleop_cmd", None):
        return shlex.split(settings.teleop_cmd), f"command: {settings.teleop_cmd}"

    teleop_dir = getattr(settings, "teleop_dir", None) \
        or os.environ.get("ORCA_TELEOP_DIR") or _find_sibling_teleop_dir()
    if not teleop_dir:
        return None, "orca_teleop checkout not found (set --teleop-dir)"
    if not os.path.isfile(os.path.join(teleop_dir, "pyproject.toml")):
...
class ChildRunner:
    ...
    def availability(self) -> dict:
        argv, detail = resolve_command(self._settings)
    ...
    def probe_cameras(self, timeout_s: float = 30.0) -> list[dict]:
        ...
        argv_prefix, detail = resolve_command(self._settings)
